...
internal class Queries : IQueries
{
    ...
    public async Task<Schema.Extremes> GetExtremes(IEnumerable<Schema.FlatArticle> dta, CancellationToken cancellationToken = default)
    {
        if (!dta.Any()) 
        ...
        do
        {
            if (enumerator.Current.PricePerUnitx100 == min)
            {
                cheapest = cheapest.Add(enumerator.Current);
            }

            if (enumerator.Current.PricePerUnitx100 == max)
            {
               mostExpensive = mostExpensive.Add(enumerator.Current);
            }
    ...
    public async Task<IEnumerable<Schema.FlatArticle>> GetMostBottles(IEnumerable<Schema.FlatArticle> dta, CancellationToken ct = default)
    {
        var maxBottles = dta.Max(x => x.NumberOfBottles);
        return dta.Where(x => x.NumberOfBottles == maxBottles);
    }

    public async Task<Schema.Dashboard> GetDashboard(IEnumerable<Schema.FlatArticle> dta, int price,  CancellationToken ct = default)
    {
